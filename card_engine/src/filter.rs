...
/// ORACLE names alone (see `crate::name_key_tier` and the route's own note) — the two surfaces
/// share a rule shape, not a scope, and conflating them would widen a route Scryfall keeps narrow.
/// They also do not share a COMPARISON: `name_key_tier` collates both sides, while this operator
/// collates the stored name against a needle the parser has already collated for it. The
/// two-halves rule is the same on both, and it arrived here and there in separate commits.
