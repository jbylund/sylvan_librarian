...
/// `None` for `Ne` (not a range — same as `numeric_candidates`); a value no card holds — a HALF a
/// real column now carries — falls out of the binary search as an empty range, with no integer
/// special case (see `numeric_candidates`).
...
fn arith_tuple_count(bounds: &[&FilterExpr], indexes: &Archived<CardIndexes>) -> Option<usize> {
    ...
    for (key, postings) in idx.keys.iter().zip(idx.postings.iter()) {
        // Same round trip through `from_bits` `arith_tuple_narrow` makes -- see ArithTupleKey.
        let bits_to_f64 = |v: &rkyv::Archived<u32>| f64::from(f32::from_bits(u32::from(*v)));
        let cmc = key.cmc_bits.as_ref().map(bits_to_f64);
        let power = key.power_bits.as_ref().map(bits_to_f64);
        let toughness = key.toughness_bits.as_ref().map(bits_to_f64);
...
//
// 2026090807 — cmc went `Option<u8>` -> `Option<f32>` on `OracleCard`, and `creature_power` /
...
const ARCHIVE_FORMAT_VERSION: u32 = 2026090807;
