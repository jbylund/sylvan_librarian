...
class APIResource:
    ...
    # Columns fetched from magic.cards to populate the Rust engine's card store.
    # Must include all filterable, partition-key, sort, prefer, and output columns.
    _ENGINE_COLUMNS: ClassVar[list[str]] = [
        "scryfall_id",
        "oracle_id",
        "illustration_id",
        "card_artist",
    ...
    def _reload_engine(self) -> None:
        ...
        if self._engine is None:
            return
        cols_sql = ", ".join(f"card.{col}" for col in self._ENGINE_COLUMNS)
        with self._conn_pool.connection() as conn:
            with conn.cursor() as cursor:
