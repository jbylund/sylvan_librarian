...
from api.parsing import ParseResult, parse_scryfall_query_with_ignored
from api.parsing.card_query_nodes import CardAttributeNode
from api.parsing.db_info import ParserClass
from api.parsing.nodes import AndNode, BinaryOperatorNode, NumericValueNode, StringValueNode


def test_parse_valid_query_with_no_ignored():
...
def test_parse_query_with_single_invalid_part():
    ...
    assert "attribute" in result.ignored[0].reason.lower()


def test_parse_query_with_multiple_valid_parts():
...
def test_parse_query_with_mixed_valid_invalid():
...
def test_parse_completely_invalid_query():
...
def test_parse_query_with_parentheses():
...
def test_parse_query_with_quoted_string():
...
def test_parse_with_or_operator():
...
def test_parse_arithmetic_expression_alone():
...
def test_parse_single_valid_attribute():
...
def test_parse_negation_with_invalid():
...
@pytest.mark.parametrize(
    ...
    argvalues=[
        ...
        ("c:g invalid pow>1", 1, True),  # Valid with invalid in middle
    ],
)
def test_parse_query_parametrized(query, expected_ignored_count, should_have_valid):
